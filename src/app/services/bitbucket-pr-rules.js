import { BitbucketCommentAction, BitbucketMergeResultOutcome, PullRequestActivityAction, PullRequestStatus } from '../models/enums';
/**
 * Check if user was added as a reviewer
 */
export function ReviewerRule(a, lastDataFetchingTimestamp, userId) {
 ...
 */
export function MergedOrDeclinedRule(a, lastDataFetchingTimestamp, userId) {
 ...
 */
export function CommentRule(a, lastDataFetchingTimestamp, userId) {
    return a.action === PullRequestActivityAction.Commented
        ...
        && (a.comment?.author.id !== userId
            ...
            || a.comment?.comments?.some(cc => cc.updatedDate >= lastDataFetchingTimestamp
                && cc.author.id !== userId))
        // check if action occurred after last running time
        && a.createdDate >= lastDataFetchingTimestamp;
}
 ...
 */
export function ConflictedRule(before, now) {
 ...
 */
export function ApprovedRule(before, now) {
    return !before.reviewers.some(r => r.approved) && now.reviewers.some(r => r.approved);
}
/**
 *  pr was approved by at least one reviewer
 */
export function NeedsWorkRule(before, now) {
...
//# sourceMappingURL=bitbucket-pr-rules.js.map